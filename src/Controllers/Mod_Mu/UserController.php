...
class UserController extends BaseController
{
    ...
    public function index($request, $response, $args)
    {
                    ...
                    static function ($query1) use ($node) {
                        if ($node->node_group != 0) {
                            if($node->port != 0 && $node->mu_only == 1){
                                $query1->where('class', '>=', $node->node_class)->where('node_group', '=', $node->node_group)->where('is_multi_user', '=', '0');
                            } else {
                            ...
                            if($node->port != 0 && $node->mu_only == 1){
                                $query1->where('class', '>=', $node->node_class)->where('is_multi_user', '=', '0');
                            } else {
                                $query1->where('class', '>=', $node->node_class);
                            }
                        }
        ...
        if($node->port != 0 && $node->mu_only == 1){
            ...
            $msg = Metron::node_Config($diffPort, $node);
