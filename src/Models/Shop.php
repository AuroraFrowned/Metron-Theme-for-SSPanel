...
class Shop extends Model
{
    ...
    public function content()
    {
        ...
        foreach ($content as $key => $value) {
            switch ($key) {
                ...
                case 'node_group': 
                    if ($value == -1) {
                        $content_text .= ', 不分配用户分组';
                    } else {
                        $content_text .= ', 用户分组变为 ' . $value;
                    }
                    break;
                default:
            }
    ...
    {
        $content = json_decode($this->attributes['content']);
        return $content->node_group ?? -1;
    }

    public function limitamount($type = 'total')
    {
        switch ($type) {
            case 'bought': 
                $Boughts = Bought::where('shopid', '=', $this->id)->where('usedd', '=', 1)->get();
                return count($Boughts) ?? 0;
            case 'can': 
                $can = $this->limitamount() - $this->limitamount('bought');
                return $can > 0 ? $can : 0;
            case 'bi':
                $num = $this->limitamount();
                if ($num === 0) return 0;

                $can = $this->limitamount('can');
                if ($can < 1) return 0;

                $bi = $can / $num * 100;
                return $bi > 0 ? round($bi, 2) : 100;
            default: 
                $content = json_decode($this->attributes['content']);
                return $content->limitamount ?? 0;
        }
    }
